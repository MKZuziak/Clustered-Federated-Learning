...
def evaluate_model(
    ...
    try:
        ...
        if node_name == None:
            ...
            evaluation_results['node_id'] = node_name
...
def automatic_generalizability_evaluation(
    iteration: int, 
    nodes: dict[int: FederatedNode],
    general_model: FederatedModel,
    save_path: str,
    logger = None,
    log_to_screen: bool = False  
):
    for node in nodes.values():
        weights = copy.deepcopy(node.get_weights())
        general_model.update_weights(weights)
        evaluate_model(
            iteration=iteration,
            model=general_model,
            save_path=save_path,
            logger=logger,
            log_to_screen=log_to_screen,
            node_name=node.model.node_name
        )