...
The following section contains demonstration of the few algorithms implemeneted by the package. All the examples can be found under the ``./demos`` directory.
...
def sattler_script():
    '''Source: https://arxiv.org/abs/1910.01991'''
    # Defining global variables
    ROOT_PATH = os.getcwd() # ROOT PATH for all the results
...
## One-Shot Federated Learning with Affinity Propagation
...
def ocfl_aff_script():
    # Defining global variables
    ROOT_PATH = os.getcwd() # ROOT PATH for all the results
    ARCHIVE_NAME = "OCFL_AFF_DEMO" # Name of the archives
    NET_ARCHITECTURE = timm.create_model('mobilenetv2_035', num_classes=10, pretrained=False, in_chans=1) # Net architecture for solving our task locally
    NUMBER_OF_CLIENTS=10 # Number of clients that will participate in the simulation.
...
## One-Shot Federated Learning with MeanShift
...
## One-Shot Federated Learning with HDBSCAN
...
## One-Shot Federated Learning with K-Means
...
def ocfl_km_script():
    # Defining global variables
    ROOT_PATH = os.getcwd() # ROOT PATH for all the results
    ARCHIVE_NAME = "OCFL_KM_DEMO" # Name of the archives
    NET_ARCHITECTURE = timm.create_model('mobilenetv2_035', num_classes=10, pretrained=False, in_chans=1) # Net architecture for solving our task locally
    NUMBER_OF_CLIENTS=10 # Number of clients that will participate in the simulation.
    NO_OF_CLUSTERS = 3
    ...
    clustering_algorithm = KMeans(n_clusters=NO_OF_CLUSTERS)
    # Step 10: Initiate the simulation (use .training_protocol_baseline)
    simulation_instace.training_protocol_energy_oneshot(
        iterations=50,
        sample_size=NUMBER_OF_CLIENTS,
